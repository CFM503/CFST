    ...
    async def tcp_ping_async(self, ip: str, semaphore: asyncio.Semaphore) -> Optional[Tuple[float, float]]:
        ...
        for i in range(self.tester.args.t):
            try:
                ...
                # 第一次超时就直接放弃该IP，不进行重试
                if i == 0 and isinstance(e, asyncio.TimeoutError):
                    if self.tester.debug:
                        print(f"      [{ip}] 第一次超时，放弃测试")
                    return None
                continue
            finally:
    ...
    def tcp_ping(self, ip: str) -> Optional[Tuple[float, float]]:
        ...
        for i in range(self.args.t):
            ...
            try:
                ...
                # 第一次超时就直接放弃该IP，不进行重试
                if i == 0 and isinstance(e, socket.timeout):
                    if self.debug:
                        print(f"      [{ip}] 第一次超时，放弃测试")
                    return None
                continue
    ...
    def http_ping(self, ip: str) -> Optional[Tuple[float, float]]:
        ...
        for i in range(self.args.t):
            ...
            try:
                ...
                # 第一次超时就直接放弃该IP，不进行重试
                if i == 0 and isinstance(e, requests.Timeout):
                    if self.debug:
                        print(f"      [{ip}] 第一次超时，放弃测试")
                    return None
                continue
